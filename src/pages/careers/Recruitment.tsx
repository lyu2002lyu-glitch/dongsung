    ...
    <div className="bg-white">
      ...
      <section className="py-32 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
        <div className="text-center mb-32">
          ...
          <h3 className="text-4xl md:text-5xl font-bold text-black mb-12 tracking-tight">동승의 인재상</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-gray-100 mb-40">
          {[
            { title: "도전", desc: "현실에 안주하지 않고 새로운 목표를 향해 나아가는 인재" },
            { title: "혁신", desc: "창의적인 사고로 변화를 주도하는 인재" },
            { title: "협력", desc: "존중과 배려를 바탕으로 팀워크를 발휘하는 인재" }
          ].map((item, idx) => (
            <div key={idx} className="bg-white p-16 text-center border-r border-gray-100 last:border-r-0 hover:bg-black hover:text-white transition-all duration-500 group">
              <h3 className="text-2xl font-bold mb-6 tracking-tight">{item.title}</h3>
              <p className="text-gray-500 group-hover:text-gray-400 leading-relaxed font-light text-sm">{item.desc}</p>
            </div>
          ))}
        </div>
        ...
        {/* Benefits Section */}
        <div className="bg-white border border-gray-100 p-16 md:p-24 mb-40">
          <div className="text-center mb-24">
            <h2 className="text-[11px] font-bold text-gray-400 uppercase tracking-[0.6em] mb-8">Benefits</h2>
            <h3 className="text-4xl font-bold text-black tracking-tight">복리후생</h3>
          </div>
        ...
        <div className="text-center">
          <h2 className="text-2xl font-bold text-black mb-12 tracking-tight">진행중인 채용공고</h2>
          <div className="bg-white border border-gray-100 p-24 text-gray-400 font-light text-sm tracking-widest uppercase">
            현재 진행중인 채용공고가 없습니다.
          </div>
        </div>
      </section>

        ...
        <div className="overflow-x-auto border-t-2 border-black">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr>
                  <td colSpan={4} className="p-12 text-center text-gray-400 text-body-m">
                    데이터를 불러오는 중입니다...
                  </td>
                </tr>
              ) : notices.length === 0 ? (
                ...
                notices.map((item, index) => (
                  ...
                  >
                    <td className="p-6 text-gray-400 text-center text-body-m">{notices.length - index}</td>
                    <td className="p-6 font-bold text-black text-body-m group-hover:text-gray-500 transition-colors">{item.title}</td>
                    <td className="p-6 text-gray-400 text-center text-body-m">{formatDate(item.created_at)}</td>
                    <td className="p-6 text-gray-400 text-center text-body-m">{item.views || 0}</td>
                  </tr>
                ))
              )}
            </tbody>
